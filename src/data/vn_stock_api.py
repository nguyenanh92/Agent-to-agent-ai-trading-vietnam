...
class VNStockAPI:
    ...
    async def get_stock_data(self, symbol: str) -> Optional[VNStockData]:
        ...
        try:
            ...
            # Method 1: VietStock (primary)
            data = await self._fetch_vietstock_data(symbol)
            
            # Method 2: CafeF (backup)
            if not data:
                data = await self._fetch_cafef_data(symbol)
            
            # Method 3: Fallback với mock data
            if not data:
    ...
    async def _fetch_vietstock_data(self, symbol: str) -> Optional[VNStockData]:
        """Fetch data từ VietStock - DISABLED due to API issues"""
        # VietStock API hiện tại không hoạt động đúng
        # Return None để fallback to mock data
        logger.warning(f"⚠️ VietStock API disabled for {symbol}, using mock data")
        return None
    
    async def _fetch_cafef_data(self, symbol: str) -> Optional[VNStockData]:
        """Fetch data từ CafeF - DISABLED due to API issues"""
        # CafeF API cũng có vấn đề tương tự
        # Return None để fallback to mock data
        logger.warning(f"⚠️ CafeF API disabled for {symbol}, using mock data")
        return None
